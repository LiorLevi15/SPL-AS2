...
public abstract class MicroService implements Runnable {
    ...
    //TODO Check: row 103 above tells us that sendEvent in msgBus shouldn't block while no one subscribed to this event!!!
    ...
    public final void run() {
        ...
        if(CRMSRunner.countDownForStudent!=null)
            CRMSRunner.countDownForStudent.countDown();
        if(CRMSRunner.countDownForTimer!=null)
            CRMSRunner.countDownForTimer.countDown();
        initialize(); //here all the subscribes are made
        while (!terminated) {
            try {
                Message msg = msgBus.awaitMessage(this);
                Callback c = callBacks.get(msg.getClass());
                c.call(msg);
